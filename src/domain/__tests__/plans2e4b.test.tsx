...
describe('plan wizard', () => {
  ...
  it('walks frequency → duration → method → schedule → review', async () => {
    renderWizard();
    fireEvent.click(await screen.findByRole('button', { name: /Continue/ }));
    expect(await screen.findByText(/How long should each conversation be/)).toBeTruthy();
    expect(screen.getByText(/£9\.00 each/)).toBeTruthy();

    fireEvent.click(screen.getByRole('button', { name: /Continue/ }));
    expect(await screen.findByText(/How would you like to talk/)).toBeTruthy();
    expect(screen.getByRole('radio', { name: /Phone call/ })).toBeTruthy();
    expect(screen.getByRole('radio', { name: /WhatsApp/ })).toBeTruthy();

    fireEvent.click(screen.getByRole('button', { name: /Continue/ }));
    expect(await screen.findByText(/Choose your weekly times/)).toBeTruthy();
  });

  it('the scheduler shows only the companion’s availability and caps at the frequency', async () => {
    renderWizard();
    fireEvent.click(await screen.findByRole('button', { name: /Continue/ })); // duration
    fireEvent.click(screen.getByRole('button', { name: /Continue/ })); // method
    fireEvent.click(screen.getByRole('button', { name: /Continue/ })); // schedule
    await screen.findByText(/Choose your weekly times/);

    // Only Tue/Thu/Sun exist; Monday is never offered.
    expect(screen.getByText('Tuesday')).toBeTruthy();
    expect(screen.getByText('Sunday')).toBeTruthy();
    expect(screen.queryByText('Monday')).toBeNull();

    // Continue is blocked until exactly `frequency` times are chosen.
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Thursday 17:00' }));
    expect(screen.getByText('2 of 3 chosen')).toBeTruthy();
    expect((screen.getByRole('button', { name: /Continue/ }) as HTMLButtonElement).disabled).toBe(true);

    fireEvent.click(screen.getByRole('button', { name: 'Sunday 10:00' }));
    expect(screen.getByText('3 of 3 chosen')).toBeTruthy();
    expect((screen.getByRole('button', { name: /Continue/ }) as HTMLButtonElement).disabled).toBe(false);
  });

  it('“Use recommended times” fills the schedule from the member’s preferences', async () => {
    renderWizard();
    fireEvent.click(await screen.findByRole('button', { name: /Continue/ }));
    fireEvent.click(screen.getByRole('button', { name: /Continue/ }));
    fireEvent.click(screen.getByRole('button', { name: /Continue/ }));
    fireEvent.click(await screen.findByRole('button', { name: /Use recommended times/ }));
    expect(await screen.findByText('3 of 3 chosen')).toBeTruthy();
  });

  it('review shows the plan, the weekly price and the honest notice — then requests it', async () => {
    renderWizard();
    fireEvent.click(await screen.findByRole('button', { name: /Continue/ }));
    fireEvent.click(screen.getByRole('button', { name: /Continue/ }));
    fireEvent.click(screen.getByRole('button', { name: /Continue/ }));
    ...
    expect((await screen.findAllByText(/Regular conversations with Daniel/)).length).toBeGreaterThan(0);
    expect(screen.getByText('3 conversations per week')).toBeTruthy();
    expect(screen.getByText('30 minutes')).toBeTruthy();
    // Weekly price = 3 × £9.00, previewed exactly as the server will snapshot it.
    expect(screen.getByText('£27.00 per week')).toBeTruthy();
    expect(screen.getByText(/Prototype plan — no payment will be taken/)).toBeTruthy();
    expect(screen.getByText(/Daniel will confirm before anything is arranged/)).toBeTruthy();

    fireEvent.click(screen.getByRole('button', { name: /Request plan/ }));
    await waitFor(() => expect(mock.rpcCalls.some((c) => c.fn === 'create_conversation_plan')).toBe(true));
    const call = mock.rpcCalls.find((c) => c.fn === 'create_conversation_plan')!;
    expect(Object.keys(call.args).sort()).toEqual([
      'p_companion', 'p_duration', 'p_frequency', 'p_member', 'p_method', 'p_slots',
    ]);
    expect(call.args.p_frequency).toBe(3);
    expect((call.args.p_slots as unknown[]).length).toBe(3);
    // No prices, credits or statuses are ever sent.
    expect(JSON.stringify(call.args)).not.toMatch(/price|credit|status|package/i);
    expect(await screen.findByText(/Daniel will confirm your plan/)).toBeTruthy();
  });

  it('duplicate submissions are prevented', async () => {
    mock.hangRpc = 'create_conversation_plan';
    renderWizard();
    fireEvent.click(await screen.findByRole('button', { name: /Continue/ }));
    fireEvent.click(screen.getByRole('button', { name: /Continue/ }));
    fireEvent.click(screen.getByRole('button', { name: /Continue/ }));
    fireEvent.click(await screen.findByRole('button', { name: /Use recommended times/ }));
    fireEvent.click(screen.getByRole('button', { name: /Continue/ }));
    const request = await screen.findByRole('button', { name: /Request plan/ });
    fireEvent.click(request);
    fireEvent.click(request);
    expect(mock.rpcCalls.filter((c) => c.fn === 'create_conversation_plan')).toHaveLength(1);
  });

  it('server rejections appear in friendly words', async () => {
    mock.rpcResults.create_conversation_plan = {
      data: null, error: { message: 'slot_unavailable: outside the companion\'s weekly availability' },
    };
    renderWizard();
    fireEvent.click(await screen.findByRole('button', { name: /Continue/ }));
    fireEvent.click(screen.getByRole('button', { name: /Continue/ }));
    ...
    fireEvent.click(await screen.findByRole('button', { name: /Use recommended times/ }));
    fireEvent.click(screen.getByRole('button', { name: /Continue/ }));
    fireEvent.click(await screen.findByRole('button', { name: /Request plan/ }));
    const alert = await screen.findByRole('alert');
    expect(alert.textContent).toMatch(/outside the companion’s weekly availability/i);
    expect(alert.textContent).not.toMatch(/slot_unavailable/);
  });

  it('never uses package, credit or purchase language', async () => {
    const view = renderWizard();
    await screen.findByRole('radio', { name: /3 per week/ });
    fireEvent.click(screen.getByRole('button', { name: /Continue/ }));
    fireEvent.click(screen.getByRole('button', { name: /Continue/ }));
    fireEvent.click(screen.getByRole('button', { name: /Continue/ }));
    fireEvent.click(await screen.findByRole('button', { name: /Use recommended times/ }));
    fireEvent.click(screen.getByRole('button', { name: /Continue/ }));
    await screen.findByText(/Weekly total/);
    expect(view.container.textContent).not.toMatch(/package|credit|purchase|checkout/i);
  });
});
