...
--
-- BUGFIX: Companions cannot submit their application — the checklist keeps
-- showing "Add a profile photo – Incomplete" (stuck at 80% = 4 of 5 required)
-- even though a photo is uploaded and visible on their profile.
--
-- Cause: profile photos now upload to `profiles.avatar_path` (public-bucket
-- change), but application_checklist still tested ONLY `photo_url`, which is now
-- always empty. So the photo item never satisfied, and submit_application (which
-- revalidates the checklist) rejected every submission.
--
-- Fix: treat the photo as present when EITHER avatar_path OR photo_url is set.
-- Body is otherwise IDENTICAL to the current version (0181): interests from the
-- profile_interests join, relaxed required set, conditional video item.
-- ===========================================================================
