-- ===========================================================================
-- 0186_companion_minimum_notice.sql
--
-- Let a companion set their own minimum booking notice (from Settings) AND make
-- the credit-booking flow actually honour it — previously get_credit_slots and
-- create_credit_booking ignored companion_profiles.minimum_notice_hours, so the
-- setting did nothing. Now slots inside the notice window are hidden and a
-- direct booking inside it is rejected ('too_soon').
-- ===========================================================================

set search_path = '';

-- Self-service get/set of the caller's own companion minimum notice (hours).
-- Pass p_hours to update (0–336), or null to just read the current value.
create or replace function public.my_companion_notice(p_hours integer default null)
returns integer language plpgsql security definer set search_path = '' as $$
declare v_profile uuid; v_val integer;
begin
  ...
  where pa.account_id = auth.uid() and pa.access_role = 'owner' and p.role = 'companion'
  order by pa.created_at limit 1;
  if v_profile is null then raise exception 'no_companion_profile' using errcode = 'P0001'; end if;

  if p_hours is not null then
    if p_hours < 0 or p_hours > 336 then raise exception 'invalid_hours' using errcode = 'P0001'; end if;
    update public.companion_profiles set minimum_notice_hours = p_hours where profile_id = v_profile;
  end if;

  select minimum_notice_hours into v_val from public.companion_profiles where profile_id = v_profile;
  return v_val;
end;
$$;
revoke all on function public.my_companion_notice(integer) from public, anon;
grant execute on function public.my_companion_notice(integer) to authenticated;

-- Redefine the slot generator to start after the companion's notice window
-- (0167 body + minimum_notice_hours).
...
declare
  ...
  v_tz text; v_notice integer; v_from timestamptz; v_to timestamptz; v_day date; v_last_day date;
...
begin
  ...
  select cp.timezone, cp.minimum_notice_hours into v_tz, v_notice
    from public.companion_profiles cp where cp.profile_id = p_companion;
  v_tz := coalesce(v_tz, 'Europe/London');
  v_from := greatest(p_from, now() + make_interval(hours => coalesce(v_notice, 0)));
...
-- Redefine create_credit_booking to reject bookings inside the notice window
-- (0185 body + notice check + booking_source = 'call_credit').
...
declare v_booked_by uuid := auth.uid(); v_booking uuid; v_notice integer;
        v_ends timestamptz := p_starts_at + interval '45 minutes';
begin
  ...
  select minimum_notice_hours into v_notice from public.companion_profiles where profile_id = p_companion_profile;
